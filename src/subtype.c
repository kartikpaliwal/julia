    ...
    if (jl_is_datatype(y)) {
        ...
        if (jl_is_datatype(x)) {
            ...
            if (istuple) {
                ...
                if (npx != npy || vx != JL_VARARG_NONE || vy != JL_VARARG_NONE) {
                    if (vy == JL_VARARG_NONE || vy == JL_VARARG_INT) { // if the bound on y is certain, we have a stronger bound on x
                        if (nparams_expanded_x >= nparams_expanded_y ||
                            vx == JL_VARARG_NONE || vx == JL_VARARG_UNBOUND) {
                            *subtype = 0;
